...
def capture_image(filename="board.png"):
    # Try multiple camera indices for Jetson
    camera_indices = [0, 1, 2] if is_jetson() else [0]
    ...
    while True:
        ...
        # Jetson display optimization
        display_frame = cv2.resize(frame, (640, 480)) if is_jetson() else frame
        cv2.imshow('Camera - Press "s" to save, "q" to quit', display_frame)
...
def extract_text_from_image(filename="board.png"):
    ...
    # Try GPU for Jetson, fallback to CPU
    try:
        reader = easyocr.Reader(["en", "ch_sim"], gpu=is_jetson())
    except Exception as e:
        print(f"GPU OCR failed, using CPU: {e}")
